...
class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, WKScriptMessageHandler {
    ...
    // Tracking state for UX specifications
    var wasActive = false
    var lastPomoPhase: String? = nil
    var doneOverrideUntil: Date? = nil
    var lastStatusJson: [String: Any]? = nil
    ...
    func handleOffline(error: Error) {
        ...
        errorCount += 1
        if errorCount >= 3 {
            statusItem.button?.image = NSImage(systemSymbolName: "exclamationmark.triangle.fill", accessibilityDescription: nil)
    ...
    func updateStatusDisplay(_ json: [String: Any]) {
        ...
        // 2-second visual Done state override
        if let doneUntil = doneOverrideUntil, Date() < doneUntil {
            return
        }
        ...
        var transitionToDone = false
        if wasActive && !active {
            transitionToDone = true
        } else if active && sessionType == "pomodoro" && wasActive {
            if lastPomoPhase == "focus" && pomoPhase == "break" {
                transitionToDone = true
            }
        }
        
        wasActive = active
        lastPomoPhase = pomoPhase
        
        if transitionToDone {
            doneOverrideUntil = Date().addingTimeInterval(2.0)
            setTitle("Done")
            statusItem.button?.image = NSImage(systemSymbolName: "checkmark.seal.fill", accessibilityDescription: nil)
            statusItem.button?.imagePosition = .imageLeft
            
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
                self?.doneOverrideUntil = nil
                if let lastJson = self?.lastStatusJson {
                    self?.updateStatusDisplay(lastJson)
                }
            }
            return
        }
        ...
        if active {
            ...
            if sessionType == "rescue" {
                statusItem.button?.image = NSImage(systemSymbolName: "lock.fill", accessibilityDescription: nil)
                statusItem.button?.imagePosition = .imageLeft
                setTitle("RESCUE")
                return
            }
            ...
            if let schedules = json["schedules"] as? [[String: Any]],
               ...
               let startTime = parseISO8601Date(startTimeStr) {
                ...
                if diff > 0 && diff <= 300 {
                    ...
                    statusItem.button?.image = NSImage(systemSymbolName: "arrow.down", accessibilityDescription: nil)
                    statusItem.button?.imagePosition = .imageLeft
                    setTitle(timeStr)
