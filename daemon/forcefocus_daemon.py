...
from sni_proxy import SniProxy
...
class ForcedFocusDaemon:
    ...
    def _cmd_request_perma_unblock(self, cmd: dict) -> dict:
        ...
        domain = self._extract_domain(cmd.get("domain", ""))
    ...
    @staticmethod
    def _extract_domain(raw_input: str) -> str:
        """Extract a clean domain from a raw URL or string input."""
        domain = raw_input.strip().lower()
        if not domain:
            return ""
            
        # Parse URL if it has a scheme
        if "://" in domain:
            parsed = urlparse(domain)
            domain = parsed.netloc or parsed.path
        else:
            # Handle cases like "youtube.com/watch?v=..."
    ...
    def _cmd_add_domains(self, cmd: dict) -> dict:
        ...
        with self.lock:
            ...
            domains = []
            for d in domains_raw:
                extracted = self._extract_domain(d)
                if self._validate_domain(extracted):
    ...
    def _enforce_whitelist(self):
        ...
        with self.enforcement_lock:
            try:
                ...
                if not getattr(self, "sni_proxy", None):
    ...
    def _start_sni_proxy(self):
        ...
        try:
            if not getattr(self, "sni_proxy", None):
                self.sni_proxy = SniProxy(self._sni_is_allowed)
                self.sni_proxy.start_sync(host="127.0.0.1", port=8443)
        except Exception as exc:
                ...
                if self.active and self.mode == "whitelist" and not is_break:
                    ...
                    rules.extend(
                        [
                            ...
                            f"rdr pass on lo0 proto tcp from any to any port 443 -> 127.0.0.1 port 8443",
    ...
    def _watchdog_tick(self):
        ...
        with self.lock:
            ...
            if not cmd_to_start and self.schedules:
                ...
                if self.schedules:
                    first_sch = self.schedules[0]
                    if first_sch.get("start_time") <= datetime.now() < first_sch.get("end_time"):
        ...
        with self.lock:
            ...
            # Integrity check: Firewall (QUIC block) every ~5s
            ...
            # Integrity check: Proxy/VPN/App Watchdog (every ~10s)
            if self._wd_persist_counter % 40 == 0:
                self._kill_restricted_apps()
                self._kill_vpn_interfaces()
...
class EmbeddedWebHandler(BaseHTTPRequestHandler):
    ...
    def _is_host_allowed(self) -> bool:
        """Reject requests with unexpected Host headers (DNS rebinding defense)."""
