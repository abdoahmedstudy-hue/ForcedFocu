...
if [[ -d "$CONFIG_DIR" ]]; then
    ...
    if [[ $HAS_PERMA -eq 1 ]]; then
        # Leave permanent_blocks.json inside so reinstall picks it up
        find "$CONFIG_DIR" -mindepth 1 -maxdepth 1 ! -name "permanent_blocks.json" -exec rm -rf {} + 2>/dev/null || true
        echo -e "    Removed: ${CONFIG_DIR} contents (preserved permanent_blocks.json)"
    else
        rm -rf "$CONFIG_DIR"
